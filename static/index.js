window.onload = async () => {
  ...
  const setMenu = (menu) => {
    ...
    let createElementsFromPlat = (plat) => {
      ...
      let url = "";
      if (plat.nutriscore.toLowerCase() === "a") {
        url = `${imgBaseUrl}Nutri-score-A.svg`;
      } else if (plat.nutriscore.toLowerCase() === "b") {
        url = `${imgBaseUrl}Nutri-score-B.svg`;
      } else if (plat.nutriscore.toLowerCase() === "c") {
        url = `${imgBaseUrl}Nutri-score-C.svg`;
      } else if (plat.nutriscore.toLowerCase() === "d") {
        url = `${imgBaseUrl}Nutri-score-D.svg`;
      } else {
        if (plat.nutriscore.toLowerCase() === "e")
      ...
      if (!plat.is_bio) {
        ...
        advertise.innerHTML =
          "Coût environnemental pour la même recette en bio : 100 pts";
